...
class sota_core(pluginTemplate):
    __model__ = "sota_core"
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Path to project root (where rtl/ and tb/ directories are)
        self.project_root = config.get('PROJECT_ROOT', os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
        if not os.path.isabs(self.project_root):
            riscof_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            self.project_root = os.path.join(riscof_dir, '..', '..', '..', self.project_root)
        self.project_root = os.path.abspath(self.project_root)

        # Path to signature extraction script
        self.sig_extract_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'extract_signature.py')
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          readelf_cmd = 'riscv{0}-unknown-elf-readelf -s {1} | grep -E "begin_signature|end_signature"'.format(self.xlen, elf)
          ...
          execute = '@cd {0}; {1}; {2}; {3}; {4}'.format(testentry['work_dir'], cmd, objcopy_cmd, objdump_cmd, simcmd)
