...
async def test_spi_reset_values(dut):
    ...
    hex_memory = {
        ...
        # Read all registers
        0x0000000C: encode_load(2, 1, 0x00),                        # LW x2, 0x00(x1) - read CTRL
        0x00000010: encode_load(3, 1, 0x04),                        # LW x3, 0x04(x1) - read STATUS
        0x00000014: encode_load(4, 1, 0x08),                        # LW x4, 0x08(x1) - read TX_DATA
        0x00000018: encode_load(5, 1, 0x0C),                        # LW x5, 0x0C(x1) - read RX_DATA
        0x0000001C: encode_load(6, 1, 0x10),                        # LW x6, 0x10(x1) - read CONFIG
        0x00000020: NOP_INSTR,
        0x00000024: NOP_INSTR,
        0x00000028: NOP_INSTR,
    }
...
async def test_spi_tx(dut):
    ...
    hex_memory = {
        ...
        0x00000034: encode_addi(7, 0, SPI_CTRL_START),                     # ADDI x7, x0, 1
        0x00000038: encode_store(1, 7, SPI_CTRL & 0xFFF),                  # SW x7, 0x00(x1) - write START
        # Wait for transfer to complete
        # Loop: poll STATUS until DONE is set
        0x0000003C: encode_load(9, 1, SPI_STATUS & 0xFFF),                 # LW x9, 0x04(x1) - read STATUS
    ...
    def callback(dut, memory):
        if dut.soc_inst.cpu_core.o_instr_addr.value == 0x0000005C:
            registers = dut.soc_inst.cpu_core.register_file.registers
...
async def test_spi_master(dut, length, end_pc):
    ...
    max_cycles = 2000000  # Need more cycles for transfer
    ...
    bfm = start_spi_slave_bfm(clk, cs_n, sclk, mosi, miso, tx_data)

    def callback(dut, memory):
        if dut.soc_inst.cpu_core.o_instr_addr.value == end_pc:
            registers = dut.soc_inst.cpu_core.register_file.registers
            ...
            for i in range(length):
                assert rx_data[i + length] == tx_data[i], \
                    f"BFM received wrong data: expected 0x{tx_data[i]:02x}, got 0x{rx_data[i + length]:02x}"
